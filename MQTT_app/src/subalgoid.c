...
// Variable d'entr�e/sortie vers algoid
...
// R�cup�re le premier message disponible dans la pile
unsigned char algo_getMessage(ALGOID destMsg, ALGOID *srcMsgStack);

// Efface un champs donn� dans la pile de message
...
// ------------------------------------------------------------------------------------
// MAIN: Point d'entr�e programme, initialisation connexion MQTT, souscription et publication
// ------------------------------------------------------------------------------------
int main(int argc, char* argv[])
{
	// Trame de test de type ALGOID � publier
    ...
    do 
    {
        ...
        if((ch=='a')){
        	...
        	//token=rand();
        	al_msgID=(short)rand();						// G�n�ration d'un ID al�atoire
    		printf("\nMessage ID: %d", al_msgID);

    		// G�n�ration des donn�es al�atoire de test
    		PAYLOAD[8]= rand() & 0x00FF;
    		PAYLOAD[15]= rand() & 0x00FF;

    		al_msgID = (al_msgID & 0x0FFF) | 0xA000;    // Ajout d'un code exp�diteur
...
// -------------------------------------------------------------------
// Fonction Call-Back r�c�ption d'un message MQTT
// -------------------------------------------------------------------
